...
try:
    ...
    with tab3:
        ...
        with st.expander("🤖 Machine Learning - Supervised (MLS)"):
            st.info("Build and evaluate supervised machine learning models (classification & regression) using only columns from your dataset. Select your target and features, then choose an algorithm.")
            ...
            numeric_cols_mls = get_numeric_columns(df)
            categorical_cols_mls = get_categorical_columns(df)
            all_cols_mls = df.columns.tolist()

            mls_task = st.radio("Select ML Task:", ["Regression", "Classification"], key="mls_task")

            if mls_task == "Regression":
                st.subheader("Regression: Predict a Numeric Target")
                target_col_reg = st.selectbox("Select numeric target column:", numeric_cols_mls, key="mls_reg_target")
                feature_cols_reg = st.multiselect("Select feature columns:", [col for col in all_cols_mls if col != target_col_reg], key="mls_reg_features")
                algo_reg = st.selectbox("Select regression algorithm:", ["LinearRegression", "RandomForestRegressor"] + (["XGBRegressor"] if xgb_available else []), key="mls_reg_algo")
                if st.button("Run Regression Model", key="mls_reg_run"):
                    if not target_col_reg or not feature_cols_reg:
                        st.warning("Please select a target and at least one feature column.")
                    else:
                        reg_df = df[[target_col_reg] + feature_cols_reg].dropna()
                        X = reg_df[feature_cols_reg]
                        y = reg_df[target_col_reg]
                        # Encode categoricals
                        X = pd.get_dummies(X, drop_first=True)
                        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)
                        if algo_reg == "LinearRegression":
                            model = LinearRegression()
                        elif algo_reg == "RandomForestRegressor":
                            model = RandomForestRegressor(n_estimators=100, random_state=42)
                        elif algo_reg == "XGBRegressor" and xgb_available:
                            model = xgb.XGBRegressor(n_estimators=100, random_state=42, verbosity=0)
                        else:
                            st.error("Selected regression algorithm is not available.")
                            model = None
                        if model:
                            try:
                                st.write(f"X_train shape: {X_train.shape}, y_train shape: {y_train.shape}")
                                if X_train.empty or y_train.empty:
                                    st.error("X_train or y_train is empty before fitting. Aborting regression.")
                                else:
                                    st.write("Attempting to fit the model...")
                                    model.fit(X_train, y_train)
                                    st.write("Model fitting completed.")

                                    # Explicitly check for estimators_ after fit
                                    if not hasattr(model, 'estimators_'):
                                        st.error("CRITICAL: 'estimators_' attribute NOT FOUND on the model after fitting!")
                                        st.info(f"Model type: {type(model)}")
                                        # Consider st.stop() here if this is unexpected
                                    else:
                                        st.write(f"Model has 'estimators_'. Number of trees: {len(model.estimators_)}")

                                    st.write("Attempting to predict...")
                                    ...
                                    st.write("Checking for feature importances...")
                                    if hasattr(model, "feature_importances_"):
                                        importances = pd.Series(model.feature_importances_, index=X_train.columns).sort_values(ascending=False)
                                        st.write("**Feature Importances:**")
                                        st.bar_chart(importances.head(15))
                                    else:
                                        st.info("The model does not have 'feature_importances_'.")
                                ...
                                st.error(f"A ValueError occurred (often due to data issues): {ve}")
                                import traceback
                                st.text(traceback.format_exc())
                            except Exception as e_fit_pred:
                                st.error(f"An unexpected error occurred during model fitting/prediction: {e_fit_pred}")
                                import traceback
                                st.text(traceback.format_exc())

            elif mls_task == "Classification":
                st.subheader("Classification: Predict a Categorical/Binary Target")
                # Allow user to binarize numeric/categorical if needed
                target_col_cls = st.selectbox("Select target column:", [col for col in all_cols_mls if df[col].nunique() <= 20 or col in categorical_cols_mls], key="mls_cls_target")
                # Binarize if not already 2 classes
                unique_vals = df[target_col_cls].dropna().unique()
                binarize = False
                if len(unique_vals) > 2:
                    st.info(f"Selected target has {len(unique_vals)} classes. You can binarize it for binary classification.")
                    binarize = st.checkbox("Binarize target (pick one class as '1', rest as '0')", key="mls_cls_binarize")
                if binarize:
                    pos_class = st.selectbox("Select positive class (will be 1):", unique_vals, key="mls_cls_posclass")
                feature_cols_cls = st.multiselect("Select feature columns:", [col for col in all_cols_mls if col != target_col_cls], key="mls_cls_features")
                algo_cls = st.selectbox("Select classification algorithm:", ["LogisticRegression", "RandomForestClassifier"] + (["XGBClassifier"] if xgb_available else []), key="mls_cls_algo")
                if st.button("Run Classification Model", key="mls_cls_run"):
                    if not target_col_cls or not feature_cols_cls:
                        st.warning("Please select a target and at least one feature column.")
                    else:
                        cls_df = df[[target_col_cls] + feature_cols_cls].dropna()
                        y = cls_df[target_col_cls]
                        if binarize:
                            y = (y == pos_class).astype(int)
                        elif y.nunique() == 2:
                            # Try to coerce to 0/1 if not already
                            if y.dtype == 'object':
                                y = (y == y.unique()[0]).astype(int)
                        X = cls_df[feature_cols_cls]
                        X = pd.get_dummies(X, drop_first=True)
                        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42, stratify=y if y.nunique() > 1 else None)
                        from sklearn.linear_model import LogisticRegression
                        if algo_cls == "LogisticRegression":
                            model = LogisticRegression(max_iter=1000)
                        elif algo_cls == "RandomForestClassifier":
                            model = RandomForestClassifier(n_estimators=100, random_state=42)
                        elif algo_cls == "XGBClassifier" and xgb_available:
                            model = xgb.XGBClassifier(n_estimators=100, random_state=42, use_label_encoder=False, eval_metric='logloss')
                        else:
                            st.error("Selected classification algorithm is not available.")
                            model = None
                        if model:
                            model.fit(X_train, y_train)
                            y_pred = model.predict(X_test)
